...
def msg_to_addr_and_pair(data):
    ip, port, user1, user2 = data.decode('utf-8').strip().split(':')
    return (ip, int(port), user1, user2)

def addr_to_msg(addr):
    return '{}:{}'.format(addr[0], str(addr[1])).encode('utf-8')
    
def addr_and_pair_to_msg(addr, pair):
    return '{}:{}:{}:{}'.format(addr[0], str(addr[1]), pair[0], pair[1]).encode('utf-8')
